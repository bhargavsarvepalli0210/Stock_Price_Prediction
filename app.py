...
def get_stock_price(ticker):
    ...
    Suppresses all unwanted error messages from yfinance.
    """
    ...
    try:
        ...
        if not history.empty:  # Ensure data is retrieved
        ...
        sys.stderr = sys.__stderr__  # Reset stderr to default behavior
...
def filter_top_100_companies(stock_prices, max_price):
    ...
    filtered_companies = [
        (company, price) for company, price in stock_prices.items() if price <= max_price
    ]
...
def stock_price():
    ...
    # Path to the file containing ticker codes
    file_path = r"E:\Stock_Price_Prediction\result.py"  # Update this path if needed

    # Load ticker codes
    ticker_codes = load_ticker_codes(file_path)
    if not ticker_codes:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        results = executor.map(get_stock_price, ticker_codes)
